...
# #         short_name = single_coin_html.find('div', {'class': 'coin-short'})
# #         price = price_container.find('span', {'class', 'pr'})
# #         url_conn = url_conn_container.find('a')
# #
# #         if name and price and url_conn and short_name:
...
# #                     svg_link=f'https://kriptomat.io/wp-content/uploads/icons/{short_name.text.lower()}.svg',
...
class KriptomatAPIService:
    __API_URL = 'https://mkt-api.kminfra.net/api/web'  # optional ?_=<UTC timestamp>

    def fetch_coins_data(self):
        coins = []
        response = requests.get(self.__API_URL)
        for property, value in json.loads(response.text)['data'].items():
            coins.append(Coin.from_json(value))
        return coins
